...
```sh
git diff --check
xcodebuild -project MyEHViewer.xcodeproj -scheme MyEHViewer -showBuildSettings | rg -n "DEVELOPMENT_TEAM|CODE_SIGN_STYLE"
xcodebuild -project MyEHViewer.xcodeproj -scheme MyEHViewer -destination 'platform=iOS Simulator,name=iPhone 17 Pro' test
```

结果：通过。App 和 ShareExtension target 均使用工程内的 Xcode Automatic Signing 配置，不再依赖额外的本地 `.xcconfig` 文件；完整测试通过。
...
结果：通过。图库详情加载失败时会继续展示本地已缓存页面入口，缓存管理页也可不依赖远端详情页直接打开缓存阅读器，已有缓存的 404 图库仍可查看本地图片；缓存管理页会显示 HTTP 404 标记，批量继续未完成下载会跳过这些图库，但不删除已有缓存；下载中遇到 404 不再做无意义重试，其它页仍会继续下载；图片请求补充移动 Safari 风格 User-Agent 和阅读页 Referer，并提高默认 HTTP 会话的单 host 连接上限；下载页任务按完成顺序补位，避免慢页阻塞后续页面入队；缓存保存路径不再每张图全量哈希整个缓存目录，并把异步保存的写盘和内容哈希移到后台任务；缓存命中读取会在后台读文件并解码成 `UIImage`，搜索、封面和目录预览会按目标尺寸降采样，减少缓存较多时的主线程卡顿、内存峰值和下载拖慢。新增 404 页面、404 详情页下载跳过测试，以及慢页不阻塞后续页面入队测试，定向测试和完整回归测试通过。
...
```sh
git diff --check
xcodebuild test -project MyEHViewer.xcodeproj -scheme MyEHViewer -destination 'platform=iOS Simulator,name=iPhone 17' -derivedDataPath /tmp/MyEHViewerFeatureBatchDerived
```

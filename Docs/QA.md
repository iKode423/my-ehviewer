...
结果：通过。覆盖搜索解析与分页、首页/热门/关注/收藏来源 URL、搜索筛选重置、保留筛选参数的搜索重试、初始关键词搜索、图库详情、标签继续搜索、带缩略图的阅读入口、缩略图分页、阅读器翻页与页码输入跳转、阅读器已知页码范围、阅读器缩放偏好、本地书架持久化、最近搜索、站点 Cookie 存储和中文文案。
